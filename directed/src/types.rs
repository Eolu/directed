...
use crate::{NodeId, node::UNNAMED_OUTPUT_NAME};
...
impl GraphOutput {
    ...
    pub(crate) fn insert(
        &mut self,
        node_id: NodeId,
        data_label: impl Into<DataLabel>,
    ...
    pub fn get<T: Any + Send + Sync>(
        ...
        self.0
            .get(&node_id)
            .and_then(|map| map.get(&data_label.into()))
    ...
    pub fn get_unnamed<T: Any + Send + Sync>(&self, node_id: NodeId) -> Option<&T> {
        ...
        self.0
            .get(&node_id)
            .and_then(|map| map.get(&UNNAMED_OUTPUT_NAME))
            .and_then(|arc| arc.downcast_ref())
    }
    ...
    pub fn take<T: Any + Send + Sync>(
        ...
        self.0
            .get_mut(&node_id)
            .and_then(|map| map.remove(&data_label.into()))
