...
impl GraphTrace {
    ...
    pub fn create_mermaid_graph(&self) -> String {
        ...
        for node in &self.nodes {
            ...
            for input in &node.inputs {
                ...
                writeln!(
                    ...
                    "        {}_in_{}[/\"{}: {type_name}\"\\]", 
                    node.id, 
                    input.name.replace(SANITIZER, "_"),
                    input.name
                ).unwrap();
            }
            
            // Define a node for each output port, unless this is a plain node.
            let has_unnamed_output = node.outputs.len() == 1 && "_" == node.outputs[0].name;
            if !(has_unnamed_output && Some(std::borrow::Cow::Borrowed("()")) == node.outputs[0].type_name) {
                for output in &node.outputs {
                    write!(&mut result, "        {}_out_{}[\\\"", node.id, output.name.replace(SANITIZER, "_")).unwrap();
                    if !has_unnamed_output {
                        write!(&mut result, "{}: ", output.name).unwrap();
                    }
        ...
        for (i, conn) in self.connections.iter().enumerate() {
            write!(&mut result, "    {}_out_{} ", conn.source_id, conn.source_output.name.replace(SANITIZER, "_")).unwrap();
            write!(&mut result, "--> ").unwrap();
            writeln!(
                &mut result,
                "{}_in_{}",
                conn.target_id,
                conn.target_input.name.replace(SANITIZER, "_")
            ).unwrap();
